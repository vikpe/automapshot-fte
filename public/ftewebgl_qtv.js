...
var wasmBinaryFile = "ftewebgl_qtv.wasm?version=20240720_0018";
    ...
    window
      .showOpenFilePicker({
        types: [
          ...
          { description: "FTE Manifest", accept: { "text/*": [".fmf"] } },
          { description: "Configs", accept: { "text/*": [".cfg", ".rc"] } },
        ],
          ...
          i.getFile()
            .then((f) => {
              f.arrayBuffer().then((arraybuf) => {
                if (FTEC.evcb.loadfile != 0) {
                  const handle =
                    _emscriptenfte_buf_createfromarraybuf(arraybuf);
...
function _emscriptenfte_pcm_loadaudiofile(
  ctx,
  callback,
  dataptr,
  datasize,
  ...
  const successcb = function (buffer) {
    ...
    if (!outptr) return;
    const dst = HEAP16.subarray(
      outptr >> 1,
      (outptr + 2 * frames * chans) >> 1,
    );
  ...
  if (gamepads !== undefined) {
    for (let i = 0; i < gamepads.length; i += 1) {
      ...
      for (let j = 0; j < gp.buttons.length; j += 1) {
        ...
        let p;
        if (typeof b == "object") p = b.pressed || b.value > 0.5;
        else p = b > 0.5;
        if (b.lastframe != p) {
          b.lastframe = p;
  ...
  if (
    ...
    if (pose) {
      ...
      if (count) {
        ...
        quat.z /= count;
        quat.w /= count;
        wasmTable.get(FTEC.evcb.jorientation)(
          -3,
          org.x,
          org.y,
          org.z,
    ...
    for (const is of FTEC.xrsession.inputSources) {
      ...
      if (is.handedness == "right") idx = -1;
        ...
        is.handedness == "none" &&
        (is.targetRayMode == "gaze" || is.targetRayMode == "screen")
      )
        idx = -4;
      else continue;
      const targetRayPose = FTEC.xrframe.getPose(
        is.targetRaySpace,
        FTEC.referenceSpace,
      );
      if (targetRayPose) {
    ...
    switch (event.type) {
              ...
              v.catch((e) => {
                if (e.name == "NotSupportedError") {
                  Module["canvas"]
                    ...
                    .then(() => {
                      console.log(
                        "Shitty browser forces mouse accel. Expect a shit experience.",
                      );
                    })
                    .catch(() => {
                      console.log(
                        "Your defective browser forces can't handle mouse look. Expect a truely dire experience. Give up now.",
                      );
                    });
                } else
                  console.log(
      ...
      case "keyup":
        if (FTEC.evcb.key != 0 && event.keyCode != 122) {
          const codepoint = event.key.codePointAt(1)
            ? 0
...
function _emscriptenfte_setupcanvas(
  ...
  try {
    ...
    window.onresize = function () {
      ...
      if (FTEC.evcb.resize != 0)
        wasmTable.get(FTEC.evcb.resize)(width * scale, height * scale, 1);
    };
    window.onresize();
...
function _emscriptenfte_xr_setup(mode) {
  ...
  if (mode == -1) _emscriptenfte_xr_shutdown();
    ...
    navigator.xr
      .requestSession(modes[mode], { optionalFeatures: ["local"] })
      .then((session) => {
...
var asmLibraryArg = {
  Z: _Sys_Clipboard_PasteText,
  m: _Sys_SaveClipboard,
  mc: ___asctime_r,
  R: ___assert_fail,
  ...
  Lc: __embind_register_constant,
  tc: __embind_register_emval,
  kb: __embind_register_float,
  Ia: __embind_register_function,
  A: __embind_register_integer,
  s: __embind_register_memory_view,
  lb: __embind_register_std_string,
  Ba: __embind_register_std_wstring,
  vc: __embind_register_void,
  Hc: __emval_as,
  Jc: __emval_decref,
  Ec: __emval_incref,
  Fc: __emval_new_array,
  Ic: __emval_new_cstring,
  Kc: __emval_new_object,
  Gc: __emval_run_destructors,
  ...
  nb: _alSourceStop,
  qb: _alSourceUnqueueBuffers,
  q: _alSourcef,
  mb: _alSourcefv,
  B: _alSourcei,
  xb: _alcCaptureCloseDevice,
  Bb: _alcCaptureOpenDevice,
  zb: _alcCaptureSamples,
  Ab: _alcCaptureStart,
  yb: _alcCaptureStop,
  Ea: _alcCloseDevice,
  sb: _alcCreateContext,
  wb: _alcDestroyContext,
  bc: _alcGetContextsDevice,
  cc: _alcGetCurrentContext,
  $b: _alcGetEnumValue,
  ac: _alcGetError,
  ha: _alcGetIntegerv,
  tb: _alcGetProcAddress,
  C: _alcGetString,
  ub: _alcIsExtensionPresent,
  Fa: _alcMakeContextCurrent,
  vb: _alcOpenDevice,
  ec: _alcProcessContext,
  dc: _alcSuspendContext,
  Ka: _clock,
  _b: _emscripten_alcDevicePauseSOFT,
  Zb: _emscripten_alcDeviceResumeSOFT,
  Yb: _emscripten_alcGetStringiSOFT,
  Xb: _emscripten_alcResetDeviceSOFT,
  X: _emscripten_asm_const_int,
  fc: _emscripten_memcpy_big,
  gc: _emscripten_resize_heap,
  xc: _emscripten_run_script_string,
  jc: _emscripten_thread_sleep,
  ka: _emscriptenfte_abortmainloop,
  Ac: _emscriptenfte_al_loadaudiofile,
  Zc: _emscriptenfte_alert,
  _c: _emscriptenfte_async_wget_data2,
  ja: _emscriptenfte_buf_create,
  Wc: _emscriptenfte_buf_delete,
  Oc: _emscriptenfte_buf_getsize,
  Nc: _emscriptenfte_buf_open,
  Mc: _emscriptenfte_buf_pushtolocalstore,
  Qc: _emscriptenfte_buf_read,
  Rc: _emscriptenfte_buf_release,
  Vc: _emscriptenfte_buf_rename,
  Pc: _emscriptenfte_buf_write,
  wc: _emscriptenfte_gl_loadtexturefile,
  Sc: _emscriptenfte_openfile,
  yc: _emscriptenfte_pcm_loadaudiofile,
  Fb: _emscriptenfte_polljoyevents,
  Yc: _emscriptenfte_print,
  La: _emscriptenfte_rtc_candidate,
  $c: _emscriptenfte_rtc_create,
  Ma: _emscriptenfte_rtc_offer,
  Qa: _emscriptenfte_setupcanvas,
  Cb: _emscriptenfte_setupmainloop,
  Gb: _emscriptenfte_updatepointerlock,
  Xc: _emscriptenfte_uptime_ms,
  Tc: _emscriptenfte_window_location,
  E: _emscriptenfte_ws_close,
  la: _emscriptenfte_ws_connect,
  _: _emscriptenfte_ws_recv,
  L: _emscriptenfte_ws_send,
  Eb: _emscriptenfte_xr_geteyeinfo,
  Pa: _emscriptenfte_xr_isactive,
  Db: _emscriptenfte_xr_issupported,
  y: _emscriptenfte_xr_setup,
  Oa: _emscriptenfte_xr_shutdown,
  Uc: _emscritenfte_buf_enumerate,
  hc: _environ_get,
  ...
  db: _glColorMask,
  Hb: _glCompileShader,
  Aa: _glCompressedTexImage2D,
  fb: _glCompressedTexSubImage2D,
  pa: _glCopyTexImage2D,
  Nb: _glCopyTexSubImage2D,
  Lb: _glCreateProgram,
  Ib: _glCreateShader,
  ab: _glCullFace,
  S: _glDeleteBuffers,
...
var _free = (Module["_free"] = function () {
  return (_free = Module["_free"] = Module["asm"]["ed"]).apply(null, arguments);
});
var _malloc = (Module["_malloc"] = function () {
  return (_malloc = Module["_malloc"] = Module["asm"]["fd"]).apply(
    null,
    arguments,
  );
});
var _main = (Module["_main"] = function () {
  return (_main = Module["_main"] = Module["asm"]["gd"]).apply(null, arguments);
});
...
var __get_tzname = (Module["__get_tzname"] = function () {
  return (__get_tzname = Module["__get_tzname"] = Module["asm"]["kd"]).apply(
    null,
    arguments,
  );
});
...
var stackAlloc = (Module["stackAlloc"] = function () {
  return (stackAlloc = Module["stackAlloc"] = Module["asm"]["nd"]).apply(
    null,
    arguments,
  );
});
var dynCall_jj = (Module["dynCall_jj"] = function () {
  return (dynCall_jj = Module["dynCall_jj"] = Module["asm"]["od"]).apply(
    null,
    arguments,
  );
});
var dynCall_jiji = (Module["dynCall_jiji"] = function () {
  return (dynCall_jiji = Module["dynCall_jiji"] = Module["asm"]["pd"]).apply(
    null,
    arguments,
  );
});
